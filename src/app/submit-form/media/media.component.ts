...
export class MediaComponent {
  ...
  removeMedia(fileInfo: any) {
    if (fileInfo.progress < 1) {
      this.fileInfos.splice(this.fileInfos.indexOf(fileInfo), 1);
    }
  }
    ...
    if (event.target.files.length > 0) {
      for (let file of event.target.files) {
        ...
        let fileinfo: any = {
          ...
          message: "Queued",
          status: "QUEUED"
        }; // TODO: make a proper class for these
        let maxGB = 5;
  ...
  private setNext() {
    return (this.nextEnabled =
      this.fileBad + this.fileDone >= this.fileInfos.length);
  }
  next() {
